...
// ========== STEP 2: Generate and Download Text File in Chunks ==========
app.post('/api/generate', async (req, res) => {
  ...
  const tempFilePath = path.join(TEMP_DIR, `${currentSessionId}.txt`);
  
  console.log(`\n📝 Request: ${owner}/${repo} | Chunk: ${chunkIndex} | Session: ${currentSessionId}`);
  ...
  try {
    if (chunkIndex === 0) {
      ...
      if (!cached || cached.repoId !== repoId) {
        return res.status(404).json({ 
          success: false, 
          error: 'Repository cache was evicted. Please run /api/analyze again.' 
        });
      }
      ...
      const fullContent = generateTextFileContent(
        selectedFiles,
        includeDirStructure,
        showLineNumbers,
        removeComments,
      ...
      fs.writeFileSync(tempFilePath, fullContent, 'utf-8');
      console.log(`📦 Compiled: ${(fullContent.length / 1024).toFixed(1)} KB -> ${path.basename(tempFilePath)}`);
    }
    ...
    const chunkContent = buffer.toString('utf-8');
...
function cleanupTempFiles() {
  ...
  try {
    ...
    try {
      ...
      if (err.code !== 'ENOENT') {
        console.log(`ℹ️ Temp directory not empty`);
      }
    }
  } catch (err) {
